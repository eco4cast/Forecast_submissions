#Script creates trained model for each site and target variable using bag_mlp
...
here::i_am("Generate_forecasts/tg_bag_mlp_all_sites/train_model.R")
...
train_site <- function(sites, noaa_past_mean, target_variable) {
  ...
  n_folds <- ifelse(nrow(site_target)<25, 5, 10)   
  ...
  bag_mlp_resamp <- vfold_cv(site_target, v = n_folds, repeats = 5, strata = site_id) #define k-fold cross validation procedure 
  ...
  best_bag_mlp<-bag_mlp_grid|>
    select_best(metric = "rmse")
  
  #select model with best tuning parameter by RMSE, cross-validation approach
  final_bag_mlp <- finalize_workflow(
    wf %>% add_model(tune_bag_mlp),
    best_bag_mlp
  )
  
  final_fit <- fit(final_bag_mlp, site_target)
  ...
  saveRDS(res_bundle, here(paste0("Generate_forecasts/tg_bag_mlp_all_sites/trained_models/", paste(theme, target_variable,"trained",Sys.Date(), sep = "-"), ".Rds")))
  tibble(theme = theme,  n_obs = nrow(site_target),n_vfolds = n_folds, target_variable = target_variable, rmse = final_rmse$.estimate, 
         penalty = best_bag_mlp$penalty, hidden_units = best_bag_mlp$hidden_units, epochs = n_epochs, 
...
mod_sums_all <- syms(apropos("_mod_summaries"))|>
  ...
  write_csv(here("Generate_forecasts/tg_bag_mlp_all_sites/model_training_summaries.csv"))
