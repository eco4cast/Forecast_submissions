...
foreach(i = 1:length(forecast_dates), 
        .packages = c("tidyverse", "tidymodels", "here", "bundle")) %dopar% {
          ...
          here::i_am("Forecast_submissions/Generate_forecasts/tg_lasso/forecast_model_reruns.R") # must declare inside foreach ex
...
forecast_site <- function(site,noaa_future_daily,target_variable) {
  ...
  if(!file.exists(here(paste0("Forecast_submissions/Generate_forecasts/tg_lasso/trained_models/",mod_file)))){
