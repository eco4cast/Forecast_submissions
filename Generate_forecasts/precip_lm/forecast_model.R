...
write.csv(noaa_past_mean,"./Generate_forecasts/noaa_downloads/past_precip.csv",row.names = F) #Save this so I don't have to rerun in tg_precip_lm_all_sites
...
forecast_site <- function(site,noaa_past_mean,noaa_future_daily,target_variable) {
  ...
  if(!target_variable%in%names(site_target)){
    ...
    fit <- lm(get(target_variable) ~ precipitation_flux, data = site_target)
    ...
    forecast <- 
      noaa_future |> 
      mutate(site_id = site,
             prediction = predict(fit, tibble(precipitation_flux)),
