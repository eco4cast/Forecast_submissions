...
library(bundle)
library(baguette)
here::i_am("Forecast_submissions/Generate_forecasts/tg_bag_tree_all_sites/forecast_model.R")
...
forecast_model <- function(target_variable,
  ...
  mod_file <- list.files(here("Forecast_submissions/Generate_forecasts/tg_bag_tree_all_sites/trained_models/"), pattern = paste(theme, target_variable, sep = "-"))
  mod_fit <- readRDS(here(paste0("Forecast_submissions/Generate_forecasts/tg_bag_tree_all_sites/trained_models/",mod_file)))
