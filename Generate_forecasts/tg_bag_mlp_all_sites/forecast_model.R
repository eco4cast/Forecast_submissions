# bag_mlp_all_sites model - regularization parameter tuned on all target historical data (date of tuning on file name in tg_bag_mlp_all_sites/trained_models/) and final fit to forecast 
...
library(bundle)
library(glmnet)
here::i_am("Generate_forecasts/tg_bag_mlp_all_sites/forecast_model.R")
...
forecast_all_sites <- function(noaa_future_daily,target_variable) {
  ...
  mod_file <- list.files(here("Generate_forecasts/tg_bag_mlp_all_sites/trained_models/"), pattern = paste(theme, target_variable, sep = "-"))
  ...
  if(nchar(mod_file)==0){
    ...
    mod_fit <- readRDS(here(paste0("Generate_forecasts/tg_bag_mlp_all_sites/trained_models/",mod_file)))
