...
noaa_future_daily <- noaa_future |> 
  ...
  # convert to Celsius
  #mutate(relative_humidity = relative_humidity - 273.15) |> 
...
write.csv(noaa_past_mean,"./Generate_forecasts/noaa_downloads/past_humidity.csv",row.names = F) #Save this so I don't have to rerun in precip_lm_all_sites

# Plot met
jpeg("met_forecasts.jpg",width = 10, height = 10, units = "in", res = 300)
ggplot(noaa_future_daily, aes(x=datetime, y=relative_humidity)) +
...
forecast_site <- function(site,noaa_past_mean,noaa_future_daily,target_variable) {
  ...
  if(!target_variable%in%names(site_target)){
    ...
    # Fit linear model based on past data: target = m * humidity + b
    fit <- lm(get(target_variable) ~ relative_humidity, data = site_target)
    
    #  Get 30-day predicted met ensemble at the site
    noaa_future <- noaa_future_daily%>%
      filter(site_id==site)
    
    # use the linear model (predict.lm) to the target variable for each ensemble member
    forecast <- 
      noaa_future |> 
      mutate(site_id = site,
             prediction = predict(fit, tibble(relative_humidity)),
