...
library(bundle)
library(ranger)
here::i_am("Forecast_submissions/Generate_forecasts/tg_randfor_parmens/forecast_model.R")
...
if(file.exists(here("Forecast_submissions/Generate_forecasts/noaa_downloads/updated_noaa_past_meteo.csv"))) {
  noaa_past_mean <- read_csv(here("Forecast_submissions/Generate_forecasts/noaa_downloads/updated_noaa_past_meteo.csv"))
} else {
...
forecast_site <- function(site,noaa_past_mean, noaa_future_daily,target_variable) {
  ...
  mod_file <- list.files(here("Forecast_submissions/Generate_forecasts/tg_randfor_parmens/trained_models/"), 
                         pattern = paste(theme, site, target_variable, sep = "-"))
  
  if(!file.exists(here(paste0("Forecast_submissions/Generate_forecasts/tg_randfor_parmens/trained_models/",mod_file)))){
    ...
    final_mod <- readRDS(here(paste0("Forecast_submissions/Generate_forecasts/tg_randfor_parmens/trained_models/",mod_file)))
