...
here::i_am("Forecast_submissions/Generate_forecasts/tg_lasso_all_sites/train_model.R")
...
train_site <- function(sites, noaa_past_mean, target_variable) {
  ...
  rec_base <- recipe(site_target)|>
    ...
    lasso_resamp <- vfold_cv(site_target, v = 10, repeats = 5, strata = site_id) #define k-fold cross validation procedure 
  ...
  #If running in parallel  
    ...
    saveRDS(res_bundle, here(paste0("Forecast_submissions/Generate_forecasts/tg_lasso_all_sites/trained_models/", paste(theme, target_variable,"trained",Sys.Date(), sep = "-"), ".Rds")))
    tibble(theme = theme,  n_obs = nrow(site_target), target_variable = target_variable, rmse = final_rmse$.estimate, lambda = best_lasso$penalty)
...
mod_sums_all <- syms(apropos("_mod_summaries"))|>
  ...
  write_csv(here("Forecast_submissions/Generate_forecasts/tg_lasso_all_sites/model_training_summaries.csv"))
