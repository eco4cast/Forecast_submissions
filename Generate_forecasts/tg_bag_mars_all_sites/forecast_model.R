# bag_mars_all_sites model - regularization parameter tuned on all target historical data (date of tuning on file name in tg_bag_mars_all_sites/trained_models/) and final fit to forecast 
...
library(bundle)
library(glmnet)
here::i_am("Generate_forecasts/tg_bag_mars_all_sites/forecast_model.R")
source(here("download_target.R"))
source(here("ignore_sigpipe.R")) #might fail locally
...
model_id = "tg_bag_mars_all_sites"
...
forecast_all_sites <- function(noaa_future_daily,target_variable) {
  message(paste0("Running ",target_variable," at all sites"))
  
  mod_file <- list.files(here("Generate_forecasts/tg_bag_mars_all_sites/trained_models/"), pattern = paste(theme, target_variable, sep = "-"))
  
  if(nchar(mod_file)==0){
    message(paste0("No trained model for var ",target_variable,". Skipping forecasts."))
    return()
    ...
    mod_fit <- readRDS(here(paste0("Generate_forecasts/tg_bag_mars_all_sites/trained_models/",mod_file)))
...
for (theme in model_themes) {
  if(!theme%in%c("beetles","ticks") | wday(Sys.Date(), label=TRUE)=="Sun"){ #beetles and ticks only want forecasts every Sunday
    ...
    ## Generate forecast
    forecast <- map_dfr(vars,possibly(~forecast_all_sites(noaa_future_daily = noaa_future_daily, target_variable = .), 
