...
forecast_site <- function(site,noaa_past_mean, noaa_future_daily,target_variable) {
  ...
  # Grab full workflow created train_model.R
...
final_mod <- readRDS(here(paste0("Forecast_submissions/Generate_forecasts/tg_randfor_ens_parm_test/trained_models/",mod_file)))
  ...
  # Save model ensemble members
 # ids <- seq(1,n_splits)
#  split_fits|>
#    map(~butcher(.)|>bundle())|>
#    map2(ids,~saveRDS(., here(paste0("Forecast_submissions/Generate_forecasts/tg_randfor_ens_parm_test/trained_models/", 
#                                     paste(theme, site, target_variable,"trained",Sys.Date(),paste0("ens", .y), sep = "-"), ".Rds"))))
  
  # Make predictions from each ensemble member to calculate errors
...
CR: might work now? look at forecast_site_test
    ###  
